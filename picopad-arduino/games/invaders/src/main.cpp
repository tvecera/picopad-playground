...
// Generate a 16-bit unsigned integer random number.
u16 RandU16() {
	return (u16) (rp2040.hwrand32() >> 16);
}
...
void DispMaxScore() {
	...
	buf[5] = 0;
	DrawTextBg(buf, MAXSCOREX + 10 * 8, SCOREY, ((MaxScore > 0) && (MaxScore == Score)) ? COL_GREEN : COL_WHITE,
						 COL_BLACK);
}
...
void loop() {
	...
	DrawClear();
	SelFont8x16();
	DrawText2("Space Invaders", (WIDTH - 14 * 16) / 2, 0, COL_YELLOW);

#define MENUX 10
#define MENUY 60
#define MENUDY 16
	DrawText("Game control:", MENUX + 16, MENUY + 0 * MENUDY, COL_GREEN);
	DrawText("LEFT/RIGHT ...... Moving", MENUX, MENUY + 1 * MENUDY, COL_WHITE);
