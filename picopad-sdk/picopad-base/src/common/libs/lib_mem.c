...
#define MEM_LOCK() irq = spin_lock_blocking((spin_lock_t *) SYS_SPIN)
#define MEM_UNLOCK() spin_unlock((spin_lock_t *) SYS_SPIN, irq)
...
void MemInit()
{
    ...
    spin_lock_unclaim(SYS_SPIN); // This ensures the lock is in the unlocked state.
}
