...
#define CS_ON    gpio_put(DISP_CS_PIN, 0); cb()    // activate chip selection
#define CS_OFF    cb(); gpio_put(DISP_CS_PIN, 1)    // deactivate chip selection
#define RES_ON    gpio_put(DISP_RES_PIN, 0); cb()    // activate reset
#define RES_OFF    cb(); gpio_put(DISP_RES_PIN, 1)    // deactivate reset
#define DC_CMD    gpio_put(DISP_DC_PIN, 0); cb()    // set command mode
#define DC_DATA    gpio_put(DISP_DC_PIN, 1); cb()    // set data mode
...
void DispWriteCmd(u8 cmd) {
    ...
    spi_write_blocking(DISP_SPI, &cmd, 1); // send command to SPI
...
void DispWriteData(const void *data, int len) {
    ...
    spi_write_blocking(spi0, data, len); // send data to SPI
    CS_OFF;    // deactivate chip selection
}
...
void DispWriteCmdData(u8 cmd, const void *data, int len) {
    ...
    spi_write_blocking(DISP_SPI, &cmd, 1); // send command to SPI
    DC_DATA;    // set data mode
    spi_write_blocking(DISP_SPI, data, len); // send data to SPI
    CS_OFF;    // deactivate chip selection
}
...
void DispHardReset() {
    ...
    sleep_ms(50);    // reset delay
    RES_OFF;    // deactivate reset
    sleep_ms(150);    // wait for initialization
    CS_OFF;    // deactivate chip selection
}
...
void DispInit(u8 rot, bool init_display_spi) {
    ...
    gpio_set_dir(DISP_BLK_PIN, GPIO_OUT);
    gpio_set_function(DISP_BLK_PIN, GPIO_FUNC_SIO);

    gpio_put(DISP_DC_PIN, 1);
    gpio_set_dir(DISP_DC_PIN, GPIO_OUT);
    gpio_set_function(DISP_DC_PIN, GPIO_FUNC_SIO);

    gpio_put(DISP_SCK_PIN, 1);
    gpio_set_dir(DISP_SCK_PIN, GPIO_OUT);
    gpio_set_function(DISP_SCK_PIN, GPIO_FUNC_SPI);

    gpio_put(DIDP_MOSI_PIN, 1);
    gpio_set_dir(DIDP_MOSI_PIN, GPIO_OUT);
    gpio_set_function(DIDP_MOSI_PIN, GPIO_FUNC_SPI);

    gpio_put(DISP_RES_PIN, 1);
    gpio_set_dir(DISP_RES_PIN, GPIO_OUT);
    gpio_set_function(DISP_RES_PIN, GPIO_FUNC_SIO);

    gpio_put(DISP_CS_PIN, 1);
    gpio_set_dir(DISP_CS_PIN, GPIO_OUT);
    gpio_set_function(DISP_CS_PIN, GPIO_FUNC_SIO);
    ...
    sleep_ms(10);
    //gpio_put(DISP_BLK_PIN, 1); // set backlight on
...
void DispTerm() {
    ...
    // terminate pins
    gpio_deinit(DISP_BLK_PIN);
    gpio_deinit(DISP_DC_PIN);
    gpio_deinit(DISP_SCK_PIN);
    gpio_deinit(DIDP_MOSI_PIN);
    gpio_deinit(DISP_RES_PIN);
    gpio_deinit(DISP_CS_PIN);
}

u8 calc_brightness(u8 value) {
...
void set_brightness(u8 value) {
    u8 bl_val = calc_brightness(value);
