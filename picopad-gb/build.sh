...
for file in roms/*; do
    if [ -f "$file" ]; then
        ...
        # Build project
        docker run -e HOME=`pwd` -u $UID -w `pwd` -v `pwd`:`pwd` -e PLATFORMIO_CORE_DIR=/app --rm -it platformiobuild pio run -e custom-loader || exit 1

        # Copy the contents of 'build' directory to 'rom_builds'
        if [ -d "build/PP2" ]; then
          cp -rf build/PP2/GAMEBOY.PP2 rom_builds/"${filename_no_ext}".PP2
        fi
